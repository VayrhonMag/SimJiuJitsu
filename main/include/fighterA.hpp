...
public:
    ...
    void externalTransition(FighterState &s, double e) const override
    {
        for (const auto &u : in->getBag())
        {
            ...
            if (s.is_attacker && s.phase == FighterState::Phase::IDLE) {
                // Si soy atacante y estaba IDLE → empezar a pensar
                s.startThinking();
            } else if (!s.is_attacker) {
                // Si no soy atacante → volver a IDLE
                s.resetAfterAction();
            }
    ...
    {
        // 🔴 CLAVE: Solo procesar si está en fase THINKING
        if (s.phase == FighterState::Phase::THINKING) {
            // Reducir tiempo de pensamiento
            s.thinking_time = 0; // Ya terminó por timeAdvance
            
            // Seleccionar técnica
            selectTechnique(s);
        }
        // 🔴 Si está en READY_TO_OUTPUT, NO HACER NADA (output se encargará)
    }

    void output(const FighterState &s) const override
    {
        // 🔴 Solo generar output cuando está READY_TO_OUTPUT y tiene técnica seleccionada
